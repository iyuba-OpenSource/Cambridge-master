...
//        paint.setColor(Color.YELLOW);
        ...
        float[] xP = new float[cor];//小圆点坐标
        float[] yP= new float[cor];
        ...
        for (int i = 0; i < cor; i++) {
            ...
            cvs.drawLine(centerX, centerY, x[i], y[i], paintGray);
            Log.d(TAG, "drawMap: "+x[i]+","+y[i]);
            if (i > 0) {
                Log.d(TAG, "drawMap:1 "+x[i]+","+y[i]);

                cvs.drawLine(x[i-1], y[i-1], x[i], y[i], paintGray);
                cvs.drawLine((4*(x[i-1]-centerX)/5)+centerX, (4*(y[i-1]-centerY)/5)+centerY, (4*(x[i]-centerX)/5)+centerX, (4*(y[i]-centerY)/5+centerY), paintGray);
                cvs.drawLine(3*(x[i-1]-centerX)/5+centerX, 3*(y[i-1]-centerY)/5+centerY, 3*(x[i]-centerX)/5+centerX, 3*(y[i]-centerY)/5+centerY, paintGray);
                cvs.drawLine(2*(x[i-1]-centerX)/5+centerX, 2*(y[i-1]-centerY)/5+centerY, 2*(x[i]-centerX)/5+centerX, 2*(y[i]-centerY)/5+centerY, paintGray);
                cvs.drawLine(1*(x[i-1]-centerX)/5+centerX, 1*(y[i-1]-centerY)/5+centerY, 1*(x[i]-centerX)/5+centerX, 1*(y[i]-centerY)/5+centerY, paintGray);
            }
            if(i == cor-1){
                cvs.drawLine(x[0], y[0], x[i], y[i], paintGray);
                cvs.drawLine((4*(x[0]-centerX)/5)+centerX, (4*(y[0]-centerY)/5)+centerY, (4*(x[i]-centerX)/5)+centerX, (4*(y[i]-centerY)/5+centerY), paintGray);
                cvs.drawLine(3*(x[0]-centerX)/5+centerX, 3*(y[0]-centerY)/5+centerY, 3*(x[i]-centerX)/5+centerX, 3*(y[i]-centerY)/5+centerY, paintGray);
                cvs.drawLine(2*(x[0]-centerX)/5+centerX, 2*(y[0]-centerY)/5+centerY, 2*(x[i]-centerX)/5+centerX, 2*(y[i]-centerY)/5+centerY, paintGray);
                cvs.drawLine(1*(x[0]-centerX)/5+centerX, 1*(y[0]-centerY)/5+centerY, 1*(x[i]-centerX)/5+centerX, 1*(y[i]-centerY)/5+centerY, paintGray);
            }
...
//            }
            ...
            int [] point = new int[result.length];

            for (int i1=0 ; i1 <result.length ; i1++) {
                Log.d(TAG, "drawMap: before"+i1);
                point[i1]=result[i1];
            }
            if (result[i] > 100) point[i] = 100;
            if (result[i] <20)   point[i] = 20;

            xP[i] = (float) (centerX + r * Math.cos(f * i + tempf) * point[i] / 100);
            yP[i] = (float) (centerY - r * Math.sin(f * i + tempf) * point[i] / 100);
            ...
            cvs.drawCircle(xP[i], yP[i], r_small, paint);//顶点小圆
            for (int i3:
                    result) {
                Log.d(TAG, "drawMap: after"+i3);
            }
        }
    }
