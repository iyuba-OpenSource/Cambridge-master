        ...
        if (mDataList.size() > util.lastAdPosition) {
            ...
            while (((ads.size() + videoAds.size()) > 0) && (mDataList.size() >= index + 4)) {
                ...
                if ((count == 1 || count == 3) && videoAds.size() > 0) {
                    ...
                    videoAds.remove(0);
                    LogUtils.e("=== ad position =11111=  " + index);
                } else if (ads.size() > 0) {
                    mDataList.add(index, ads.get(0));
                    ads.remove(0);
                    LogUtils.e("=== ad position =22222=  " + index);
...
//                    L.e("=== ad position =33333=  " + index);
        ...
        multiNative = new YouDaoMultiNative(context, "3438bae206978fec8995b280c49dae1e", new YouDaoMultiNative.YouDaoMultiNativeNetworkListener() {
            ...
            public void onNativeLoad(NativeAds nativeAds) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    LogUtils.e(responses.get(i).toString());
                }
                isAdReponse = true;
                LogUtils.e("info flow youDao ad  onNativeLoad ------");
                callback(callback);
            }

            @Override
            public void onNativeFail(NativeErrorCode nativeErrorCode) {
                LogUtils.e("info flow youDao: onNativeFail :  " + nativeErrorCode.name() + "  --  Message : " + nativeErrorCode.toString());
                isAdReponse = true;
                callback(callback);
            }
        });
    ...
    public void refreshAd() {
        ...
        LogUtils.e("info flow refresh ad ------");
        isAdReponse = false;
