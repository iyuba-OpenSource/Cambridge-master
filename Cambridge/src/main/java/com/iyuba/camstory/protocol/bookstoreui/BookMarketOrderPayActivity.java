...
import com.iyuba.camstory.pay.NoScrollListView;
import com.iyuba.camstory.pay.OrderGenerateRequest;
import com.iyuba.camstory.pay.PayMethodAdapter;
import com.iyuba.camstory.pay.PayResult;
import com.iyuba.camstory.pay.RequestCallBack;
...
public class BookMarketOrderPayActivity extends SwipeBackActivity {
...
//        price = 0.01f;
        for (int i = 0; i < bookDetails.size(); i++) {
            price = price + bookDetails.get(i).num * Double.parseDouble(bookDetails.get(i).totalPrice);
        ...
        payorder_submit_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (AccountManager.getInstance().islinshi) {
                    startActivity(new Intent(BookMarketOrderPayActivity.this, LoginActivity.class));
...
//                            break;
    ...
    private void payByAlipay(String body, String subject) {
        ...
        OrderGenerateRequest orderRequest = new OrderGenerateRequest(productId, Seller, out_trade_no,
                ...
                String.valueOf(AccountManager.getInstance().userId), String.valueOf(totalAmount),
...
//            }
//        };
//        String uid = String.valueOf(AccountManager.Instance(mContext).userId);
//        OrderGenerateWeiXinRequest request = new OrderGenerateWeiXinRequest(productId, mWeiXinKey, Constant.APPID, uid, String.valueOf(price), String.valueOf(totalAmount),body, mOrderErrorListener, rc);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0: {
                    ...
                    for (int i = 0; i < bookDetails.size(); i++) {
                        mDB.delete("shop_cart", "id=? and uid=?", new String[]{bookDetails.get(i).id, AccountManager.getInstance().userId+""});
                    }
