 ...
 */
//自定义的下载进度圈
//思路:如果要自定义一个下载进度圈.要知道两点:
// 第一:最大值,当前进度,
//第二:画出一个圈,然后根据其进度为其着色,着色为其绘制background吗?
public class ProgressWheel extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        layout_width = w;
        layout_height = h;
//当view的大小发生改变的时候在这个方法里面处理逻辑就是设置一下画笔的属性
        setupBounds();
        setupPaints();
        invalidate();
    }
...
//    自定义方法
    ...
    int autoAnimateDelay;
    onAnimateFinishedListener animateFinishedListener;
    //自动动画
    public void autoAnimate(int totalMillis) {
    	autoAnimateDelay = totalMillis/360;
    	animateHandler.removeMessages(0);
    	animateHandler.sendEmptyMessage(0);
    }
    
    public void setAnimateFinishedListener(onAnimateFinishedListener listener){
    	this.animateFinishedListener = listener;
    }
    ...
    private Handler animateHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            progress++;
                if (progress > 360) {
                    progress = 0;
                    if (animateFinishedListener != null) {
                    	animateFinishedListener.onAnimateFinished();
					}
                }
                else {
					sendEmptyMessageDelayed(0, autoAnimateDelay);
				}
        }
    };
