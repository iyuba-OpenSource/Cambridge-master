...
public class DownloadThread extends Thread {
	...
	public DownloadThread(FileDownloader downloader, URL downUrl, File saveFile,
			int block, int downLength, int threadId, int bufferSize) {
		print("run DownloadThread constructor");
	...
	public void run() {
		if (downLength < block) {
			try {
				...
				int offset = 0;
				print("Thread " + this.threadId + " start download from position "
						+ startPos);
				...
				print("Thread " + this.threadId + "  endposition : "
						+ (block * (threadId - 1) + downLength));
