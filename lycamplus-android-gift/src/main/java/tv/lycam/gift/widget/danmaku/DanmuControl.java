package tv.lycam.gift.widget.danmaku;
...
public class DanmuControl {
    ...
    //弹幕背景色
    private static final int PINK_COLOR = 0x99000000;// 楼主
    private static final int ORANGE_COLOR = 0x99ff5a93;// 我
    private static final int BLACK_COLOR = 0x99000000;// 普通

    //    private int BITMAP_WIDTH = 30;
//    private int BITMAP_HEIGHT = 30;
    private float DANMU_TEXT_SIZE = 10f;//弹幕字体的大小
    private int AVATAR_SIZE = 20;//头像的大小
...
//        EMOJI_SIZE = ScaleUtil.dip2px(ontext, EMOJI_SIZE);
    ...
    private void initDanmuConfig() {
        ...
        maxLinesPair.put(BaseDanmaku.TYPE_SCROLL_RL, 4); // 滚动弹幕最大显示x行
        // 设置是否禁止重叠
...
//            danmaku.padding = 20;  // 在背景绘制模式下增加padding
        ...
        public void drawBackground(BaseDanmaku danmaku, Canvas canvas, float left, float top) {
            ...
            if (!danmaku.isGuest && danmaku.userId == mGoodUserId) {
                paint.setColor(BLACK_COLOR);//粉红 楼主
            } else if (!danmaku.isGuest && danmaku.userId == mMyUserId
                    && danmaku.userId != 0) {
                paint.setColor(BLACK_COLOR);//橙色 我
            } else {
    ...
    public void addDanmu(final Danmu danmu) {
         ...
         * 则直接发送，否则设置间隔
         */
        if (mDanmakuView.getCurrentVisibleDanmakus().size() < 10 && preDanmuTime < mDanmakuView.getCurrentTime()) {
        ...
        final SimpleTarget<Bitmap> simpleTarget = new SimpleTarget<Bitmap>(20, 20) {
            @Override
            ...
            public void onLoadFailed(Exception e, Drawable errorDrawable) {
                ...
                final SimpleTarget<Bitmap> failedTarget = new SimpleTarget<Bitmap>(20, 20) {
                    @Override
                    public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                ...
                Glide.with(mContext.getApplicationContext())
                        .load(R.drawable.icon_default_user)
                        .asBitmap()
                        .transform(new CropCircleTransformation(mContext.getApplicationContext()))
                        .into(failedTarget);
            }
        };
